...
namespace TheLibrary.Application.Services
{
    public class ManageLoginService : IManageLoginService
    {
        ...
        public ManageLoginService(IUnitOfWork uow, ITokenService tokenService)
        {
            _uow = uow;
        ...
        public async Task<string> Login(LoginDTO credentials)
        {
            ...
            var user = await _uow.Repository<User>().Get(w => w.Login.Equals(credentials.Login) && w.Password.Equals(credentials.Password));
